...
var sqlMap = {
  ...
  stu: {
    ...
    addTrainingSbjPl:'insert into trainingsubjectplan(schoolYear, term, class, stuName, subject, teacher, createDate, updateDate, teachingPlan, teachingProgress) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
    ...
    upTrainingSbjPl:'update trainingsubjectplan set schoolYear=?, term=?, class=?, stuName=?, subject=?, teacher=?, createDate=?, updateDate=?, teachingPlan=?, teachingProgress=? where id = ? and isDel = 0', 
    ...
    queClassGSP:'select * from groupsubjectplan where class = ? and term = ? and isDel = 0',
